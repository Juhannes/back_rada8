...
public interface AdvertisementRepository extends JpaRepository<Advertisement, Integer> {
    ...
    @Query("select a from Advertisement a where a.city.id = ?1 and a.status = ?2")
    List<Advertisement> findByCityIdAndStatus(Integer id, String status);

    @Query("select a from Advertisement a where a.type.id = ?1 and a.status = ?2")
    List<Advertisement> findByTypeIdAndStatus(Integer id, String status);
